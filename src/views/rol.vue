      ...
      <v-app>
        <v-card-actions>
          ...
          <v-dialog v-if="!dialog" max-width="600px" loading>
            ...
            <v-card>
              <v-card-title>
                <span class="text-h5">Editar Rol a partir de clave</span>
              </v-card-title>
              <v-card-text>
                <v-form>
                  <v-container>
...
export default {
  ...
  created() {
    ...
    axios.get("http://localhost:8000/admrol.php").then((res) => {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      axios
        .post("http://localhost:8000/rolpost.php", formData)
        .then((response) => {
    ...
    onUpdate(clave) {
      ...
      var formData = this.toFormData(this.enteredData);
      axios
        .post("http://localhost:8000/rolUpdate.php", formData)
        .then((response) => {
          if (response.data.error) {
            this.snackbar=true;
            this.message=("Danger: "+ response.data.message.toString() );
    ...
    onDelete(clave) {
      ...
      var formData = this.toFormData(this.enteredData);
      axios
        .post("http://localhost:8000/rolDelete.php", formData)
        .then((response) => {
          if (response.data.error) {
            this.snackbar=true;
            this.message=("Danger: "+ response.data.message.toString() );

...
<v-dialog v-if="!dialog" max-width="600px" loading>
              ...
              <v-card>
                <v-card-title>
                  <span class="text-h5">Agregar pais</span>
                </v-card-title>
                <v-card-text>
                  <v-form>
                    <v-container>
...
export default {
  ...
  created() {
    ...
    axios.get("http://localhost:8000/admpais.php").then((res) => {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      axios
        .post("http://localhost:8000/paispost.php", formData)
        .then((response) => {
    ...
    onUpdate(clave) {
      ...
      var formData = this.toFormData(this.enteredData);
      axios
        .post("http://localhost:8000/paisUpdate.php", formData)
        .then((response) => {
          if (response.data.error) {
            this.snackbar=true;
            this.message=("Danger: "+ response.data.message.toString() );
    ...
    onDelete(clave) {
      ...
      var formData = this.toFormData(this.enteredData);
      axios
        .post("http://localhost:8000/paisDelete.php", formData)
        .then((response) => {
          if (response.data.error) {
            this.snackbar=true;
            this.message=("Danger: "+ response.data.message.toString() );
